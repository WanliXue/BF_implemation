...
class BF_TS:
    def __init__(self, bf_len, bf_num_hash_func, bf_num_inter, bf_step,
        ...
        self.bf_step =round(bf_step,3)   # only keep 2 decimal
        # self.bf_step = 0.5
    ...
    # convert cs bf input to no neighboured bf
    ...
    def add_neibour(self,val1, sequence):
        ...
        # Add variations larger and smaller than the actual value
        #
        # for i in range(bf_num_inter + 1): #if +1 will comes to 12  updated june
        for i in range(bf_num_inter ):
            diff_val = (i + 1) * bf_step
            left_value=use_val1 - diff_val
            if float("{0:.2f}".format(left_value)) == 0:
                left_value = 0.000
            val1_set.add(str("{0:.2f}".format(left_value)) + str(sequence))

            diff_val = (i) * bf_step
            right_value = use_val1 + diff_val
            if float("{0:.2f}".format(right_value)) == 0:
                right_value = 0.0000
            val1_set.add(str("{0:.2f}".format(right_value)) + str(sequence))
