...
#     normalise the data and add to the list for encoding to the bf
    ...
    def add_neibour(self,val1, sequence):
        ...
        val1_set.add(str("{0:.3f}".format(float(use_val1))) + str(sequence))  # Add the actual value
        ...
        for i in range(bf_num_inter):
            ...
            if float("{0:.3f}".format(left_value)) == 0:
                left_value = 0.000
            val1_set.add(str("{0:.3f}".format(left_value)) + str(sequence))

            diff_val = (i) * bf_step
            right_value = use_val1 + diff_val
            if float("{0:.3f}".format(right_value)) == 0:
                right_value = 0.0000
            val1_set.add(str("{0:.3f}".format(right_value)) + str(sequence))
